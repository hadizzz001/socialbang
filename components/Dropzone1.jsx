...
import { useMyContext } from '../app/context/PDFContext';

const Dropzone1 = ({ className }) => {
  ...
  // useEffect(() => { 
  //   localStorage.setItem("pdf", JSON.stringify(inputs))
        ...
        <ul className='mt-6 grid grid-cols-1 gap-10 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6'>
          {files.map(file => (
            <li key={file.name} className='relative h-32 rounded-md shadow-lg'>
              <Image
                src='https://res.cloudinary.com/duppvjinz/image/upload/v1704379650/t4ytgtze5im6nb50vgna.png'
                alt={file.name}
                width={100}
                height={100}
                onLoad={() => {
